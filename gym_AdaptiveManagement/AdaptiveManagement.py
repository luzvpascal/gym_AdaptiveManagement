...
class AdaptiveManagement(gym.Env):
    ...
    def __init__(self, transition_models, reward_function, #true_model_index=0,
        ...
        self.N_transition_models = self.transition_models.shape[0]
        self.reward_function = reward_function

        self.belief = np.ones(self.N_transition_models)/self.N_transition_models
        self.time_step = 0

        self.true_model_index = np.random.choice([0,1], 1, p=self.belief)[0]
        self.true_transition_model = self.transition_models[self.true_model_index]

        self.Tmax = Tmax

        self.observation_space = spaces.Dict({
                                        "state": spaces.Discrete(2),
                                        "belief": spaces.Box(0, 1, shape=(self.N_transition_models,))
                                        #,
    ...
    def update_belief(self,action,new_observation,past_observation):
      ...
      new_belief = np.zeros(self.N_transition_models)
      for k in range(self.N_transition_models):
        new_belief[k] = self.transition_models[k][action][past_observation][new_observation]*self.belief[k]
